...
var PayloadSchema3 = exports_external.object({
  service_target: exports_external.string(),
  queryParameters: exports_external.record(exports_external.string(), exports_external.any()),
  paymentPayload: exports_external.string()
});
var onHttpTrigger = async (runtime2, request) => {
  const parsed = PayloadSchema3.safeParse(request.payload);
  if (!parsed.success) {
    runtime2.log("Invalid payload received.");
    return {
      status: "error",
  ...
  runtime2.log(`Received request for ${payload.service_target} with payment payload: ${payload.paymentPayload.substring(0, 10)}...`);
  const facilitatorUrl = runtime2.config.facilitatorUrl || "https://facilitator.x402.local/verify";
  const httpClient = new ClientCapability;
  runtime2.log(`Verifying payment payload with x402 facilitator at ${facilitatorUrl}...`);
  const verifyRes = await httpClient.sendRequest("POST", facilitatorUrl, {
    "Content-Type": "application/json"
  }, JSON.stringify({ paymentPayload: payload.paymentPayload }));
  if (verifyRes.statusCode !== 200) {
    runtime2.log("Payment verification failed.");
    return {
      status: "error",
      message: "Payment verification failed"
    };
  }
  runtime2.log("Payment verified securely.");
  return {
    status: "success",
