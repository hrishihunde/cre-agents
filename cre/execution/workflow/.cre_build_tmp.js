...
var onLogTrigger = (runtime2, log) => {
  ...
  try {
    resultData = decodeJson(serviceRes.body);
  } catch {
    resultData = new TextDecoder().decode(serviceRes.body);
  }
  const evmClient = new ClientCapability(BigInt(runtime2.config.chain_selector));
  const agentContract = new X402Agent(evmClient, runtime2.config.contract_address);
  agentContract.writeReportFromFulfillService(runtime2, event.agent, resultData);
  return {};
};
var initWorkflow = (config) => {
  const evmClient = new ClientCapability(BigInt(config.chain_selector));
