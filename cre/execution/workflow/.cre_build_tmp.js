...
`), {
  ...
  AbiEventNotFoundError = class AbiEventNotFoundError2 extends BaseError {
...
var X402AgentABI = [{ anonymous: false, inputs: [{ indexed: true, internalType: "address", name: "agent", type: "address" }, { indexed: false, internalType: "string", name: "serviceUrl", type: "string" }, { indexed: false, internalType: "string", name: "paymentPayload", type: "string" }], name: "ServiceRequested", type: "event" }, { inputs: [{ internalType: "address", name: "agent", type: "address" }, { internalType: "string", name: "resultData", type: "string" }], name: "fulfillService", outputs: [], stateMutability: "nonpayable", type: "function" }];
...
class X402Agent {
  ...
  writeReportFromFulfillService(runtime2, agent, resultData, gasConfig) {
    const callData = encodeFunctionData({
      abi: X402AgentABI,
      functionName: "fulfillService",
      args: [agent, resultData]
    });
    const reportResponse = runtime2.report(prepareReportRequest(callData)).result();
    return this.client.writeReport(runtime2, {
      receiver: this.address,
      report: reportResponse,
      gasConfig
    }).result();
  }
  writeReport(runtime2, callData, gasConfig) {
    const reportResponse = runtime2.report(prepareReportRequest(callData)).result();
    return this.client.writeReport(runtime2, {
      receiver: this.address,
      report: reportResponse,
  ...
  logTriggerServiceRequested(filters) {
    let topics;
    if (!filters || filters.length === 0) {
      ...
      topics = allEncoded[0].map((_, i2) => ({
        values: [...new Set(allEncoded.map((row) => hexToBase64(row[i2])))]
      }));
    }
    ...
    return {
      ...
      adapt: (rawOutput) => contract.decodeServiceRequested(rawOutput)
    };
  }
  decodeServiceRequested(log) {
    const decoded = decodeEventLog({
      abi: X402AgentABI,
      data: bytesToHex(log.data),
      topics: log.topics.map((t) => bytesToHex(t))
    });
...
var onLogTrigger = (runtime2, log) => {
  ...
  const verifyRes = httpClient.sendRequest(runtime2, {
    request: {
  ...
  const serviceRes = httpClient.sendRequest(runtime2, {
    request: {
  ...
  const evmClient = new ClientCapability(runtime2.config.chain_selector.toString());
  const agentContract = new X402Agent(evmClient, runtime2.config.contract_address);
  agentContract.writeReportFromFulfillService(runtime2, event.agent, serviceRes.body || "x402 Execution Fulfilled");
};
var initWorkflow = (config) => {
  const evmClient = new ClientCapability(config.chain_selector.toString());
  const agentContract = new X402Agent(evmClient, config.contract_address);
  return [
    handler(agentContract.logTriggerServiceRequested(), onLogTrigger)
  ];
};
async function main() {
